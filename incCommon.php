	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'students' => array('Students', 'student details', 'resources/table_icons/group.png', 'None'),
			'units' => array('Units', 'list of all units', 'resources/table_icons/books.png', 'None'),
			'courses' => array('Courses', 'list of all courses', 'resources/table_icons/attributes_display.png', 'None'),
			'attendance' => array('Attendance Record', 'contains attendance record of students', 'resources/table_icons/application_view_icons.png', 'None'),
			'Marks' => array('Marks', 'Contains marks submitted by the lecturer for different assesments', 'resources/table_icons/chart_line_add.png', 'None'),
			'academic_year' => array('Academic year', 'academic years', 'resources/table_icons/check_box.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'students' => "`students`.`regno` as 'regno', `students`.`name` as 'name', IF(    CHAR_LENGTH(`courses1`.`name`), CONCAT_WS('',   `courses1`.`name`), '') as 'course', `students`.`year` as 'year', IF(    CHAR_LENGTH(`academic_year1`.`name`), CONCAT_WS('',   `academic_year1`.`name`), '') as 'academicyear'",
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'students' => "`students` LEFT JOIN `courses` as courses1 ON `courses1`.`id`=`students`.`course` LEFT JOIN `academic_year` as academic_year1 ON `academic_year1`.`id`=`students`.`academicyear` ",
			'units' => "`units` ",
			'courses' => "`courses` ",
			'attendance' => "`attendance` LEFT JOIN `students` as students1 ON `students1`.`regno`=`attendance`.`student` LEFT JOIN `units` as units1 ON `units1`.`id`=`attendance`.`unit` LEFT JOIN `academic_year` as academic_year1 ON `academic_year1`.`id`=`students1`.`academicyear` ",
			'Marks' => "`Marks` LEFT JOIN `units` as units1 ON `units1`.`id`=`Marks`.`unit` LEFT JOIN `students` as students1 ON `students1`.`regno`=`Marks`.`student` LEFT JOIN `academic_year` as academic_year1 ON `academic_year1`.`id`=`students1`.`academicyear` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Student_Management_System_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Student_Management_System_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Student_Management_System_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Student_Management_System_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Student Management System</a>
			</div>
